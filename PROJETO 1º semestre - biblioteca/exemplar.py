...
# Modelo
class Exemplar:
  def __init__(self, id, editora, ano, idLivro):
    self.id = id
    self.editora = editora
    self.ano = ano
    self.idLivro = idLivro
  def __str__(self):
    return f"{self.id} - {self.editora} - {self.ano}"

def to_json(self):
    dic = {}
    dic["id"] = self.id
    dic["editora"] = self.editora
    dic["ano"] = self.ano
  ...
  def abrir(cls):
    ...
    try:
      with open("exemplares.json", mode="r") as arquivo:   # r - read 
        ...
        for obj in texto:   
          c = Exemplar(obj["id"], obj["editora"], obj["ano"], obj["idLivro"])
